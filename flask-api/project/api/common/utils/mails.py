...
def send_password_recovery_email(user, token):
    ...
    send_async_password_recovery_email.delay(subject="Password Recovery by Flask Base Api %s" % user.email,
...
def send_registration_email(user, token):
    ...
    send_async_registration_email.delay(subject="Welcome by Flask Base Api! %s" % user.email,
                                        recipient=user.email,
...
def send_email_verification_email(user, token):
    href = request.base_url + '/' + token
    send_async_email_verification_email.delay(subject="Email confirmation by Flask Base Api! %s" % user.email,
                                              recipient=user.email,
