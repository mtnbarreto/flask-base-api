...
class TestTestingConfig(TestCase):
    ...
    def test_app_is_testing(self):
        ...
        self.assertTrue(
            current_app.config['SQLALCHEMY_DATABASE_URI'] == os.environ.get('DATABASE_TEST_URL')
        )
        self.assertTrue(current_app.config['BCRYPT_LOG_ROUNDS'] == 4)
        self.assertTrue(current_app.config['TOKEN_EXPIRATION_DAYS'] == 0)
        self.assertTrue(current_app.config['TOKEN_EXPIRATION_SECONDS'] == 3)
...
class TestProductionConfig(TestCase):
    ...
    def test_app_is_production(self):
        ...
        self.assertTrue(current_app.config['BCRYPT_LOG_ROUNDS'] == 13)
